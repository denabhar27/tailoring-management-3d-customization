...
const getSizeRentalCountDetails = (item) => {
  if (!item) {
    return {
      totalCompleted: 0,
      sizeBreakdown: 'No completed rentals per size yet.',
      sizeBreakdownMultiline: 'No completed rentals per size yet.'
    };
  }

  const countsRaw = item.size_rental_counts;
  let counts = countsRaw;
  ...
  let parsedSize = null;
  try {
    parsedSize = typeof item.size === 'string' ? JSON.parse(item.size) : item.size;
  } catch {
    parsedSize = null;
  }

  const labelMap = new Map();
  if (parsedSize && Array.isArray(parsedSize.size_entries)) {
    parsedSize.size_entries.forEach((entry) => {
      const key = String(entry?.sizeKey || entry?.size_key || '').trim();
      if (!key) return;
      const label = String(entry?.label || SIZE_LABELS[key] || key).trim();
      labelMap.set(key, label);
    });
  }

  const sizeRows = Object.entries(counts)
    .map(([sizeKey, qtyRaw]) => {
      const qty = Math.max(0, parseInt(qtyRaw, 10) || 0);
      if (qty <= 0) return null;
      const label = labelMap.get(sizeKey) || SIZE_LABELS[sizeKey] || sizeKey;
      return { label, qty };
    })
    .filter(Boolean);

  const sizeCountTotal = Object.values(counts).reduce((sum, qtyRaw) => {
    return sum + Math.max(0, parseInt(qtyRaw, 10) || 0);
  }, 0);

  const timesRentedRaw = Math.max(0, parseInt(item.times_rented, 10) || 0);
  const totalCompleted = Math.max(timesRentedRaw, sizeCountTotal);

  const detailedRows = sizeRows.map((row) => {
    const suffix = row.qty === 1 ? 'rental' : 'rentals';
    return `${row.label}: ${row.qty} completed ${suffix}`;
  });

  return {
    totalCompleted,
    sizeBreakdown: detailedRows.length > 0 ? detailedRows.join('; ') : 'No completed rentals per size yet.',
    sizeBreakdownMultiline: detailedRows.length > 0
      ? detailedRows.map((row, idx) => `${idx + 1}. ${row}`).join('\n')
      : 'No completed rentals per size yet.'
  };
};

const getSizeRentalCountSummary = (item) => {
  const details = getSizeRentalCountDetails(item);
  return details.sizeBreakdown;
};
...
const PostRent = () => {
  ...
  const saveItem = async () => {
    ...
    if (!String(formData.item_name || '').trim()) validationErrors.push('Item name is required.');
    if (!String(formData.category || '').trim()) validationErrors.push('Category is required.');
    if (!String(formData.color || '').trim()) validationErrors.push('Color is required.');
    if (!String(formData.material || '').trim()) validationErrors.push('Material is required.');

    const sizeEntries = formData.size_entries || [];
    const activeEntries = sizeEntries.filter(entry => (parseInt(entry.quantity, 10) || 0) > 0);

    if (activeEntries.length === 0) {
      validationErrors.push('At least one size entry with quantity greater than 0 is required.');
    }

    activeEntries.forEach((entry, index) => {
      const rowNo = index + 1;
      if (entry.sizeKey === 'custom' && !String(entry.customLabel || '').trim()) {
        validationErrors.push(`Size row ${rowNo}: custom size label is required.`);
      }

      const parsedPrice = parseFloat(entry.price);
      if (!Number.isFinite(parsedPrice) || parsedPrice <= 0) {
        validationErrors.push(`Size row ${rowNo}: price must be greater than 0.`);
      }

      const parsedDeposit = parseFloat(entry.deposit);
      if (!Number.isFinite(parsedDeposit) || parsedDeposit < 0) {
        validationErrors.push(`Size row ${rowNo}: deposit must be 0 or greater.`);
      } else if (Number.isFinite(parsedPrice) && parsedPrice > 0 && parsedDeposit > parsedPrice) {
    ...
    if (formData.status === 'maintenance' && !String(formData.damage_notes || '').trim()) {
      validationErrors.push('Damage/Maintenance notes are required when status is Maintenance.');
    }

    // Description and care instructions are optional by request.
    if (validationErrors.length > 0) {
      const validationMessage = `Please complete the required fields:\n\n- ${validationErrors.join('\n- ')}`;
      setError(validationMessage);
      await alert(validationMessage, 'Validation Error', 'error');
      return;
    }
            ...
            filteredItems.map(item => (
              ...
              <div key={item.item_id} className="compact-item-card" onClick={() => openDetailModal(item)}>
                ...
                <div className="compact-item-info">
                  ...
                  <div className="compact-price-block">
                  ...
                  <div className="compact-rent-stats-block">

                    <div style={{ fontSize: '10px', color: '#5f6368', marginBottom: '6px', lineHeight: '1.35' }}>
                      Completed Rentals: {getSizeRentalCountDetails(item).totalCompleted}
                      <br />
                      Completed Rental Breakdown by Size: {getSizeRentalCountDetails(item).sizeBreakdown}
                    </div>
          ...
          <div className="detail-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="detail-modal-body">
              ...
              <div className="detail-info-section">
                ...
                <div className="detail-rent-stats">

                  <h4>Size Rent Counts</h4>
                  ...
                  <div className="price-grid" style={{ gridTemplateColumns: '1fr' }}>
                    ...
                    <div className="price-item">
                      ...
                      <span className="price-value" style={{ whiteSpace: 'normal', textAlign: 'left', fontSize: '12px' }}>{getSizeRentalCountDetails(selectedItem).totalCompleted}</span>

                    </div>

                    <div className="price-item">

                      <label>Completed Rental Breakdown by Size</label>

                      <span className="price-value" style={{ whiteSpace: 'pre-line', textAlign: 'left', fontSize: '12px' }}>{getSizeRentalCountDetails(selectedItem).sizeBreakdownMultiline}</span>
