...
export const orderTrackingService = {
  ...
  hagglePrice: async (orderItemId: string, offeredPrice: number) => {
    return apiCall(`/orders/${orderItemId}/haggle-price`, {
      method: 'POST',
  ...
  requestEnhancement: async (
    ...
    if (photos.length > 0) {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/tracking/request-enhancement/${orderItemId}`, {
          method: 'POST',
          headers: {
            Authorization: token ? `Bearer ${token}` : '',
          },
  ...
  confirmOrderItemPickup: async (orderItemId: string) => {
    return apiCall(`/tracking/confirm-pickup/${orderItemId}`, {
      method: 'POST',
    });
  }
};
