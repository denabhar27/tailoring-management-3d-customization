...
export default function ProfileScreen() {
  ...
  const [submittingHaggleByItem, setSubmittingHaggleByItem] = useState<Record<number, boolean>>({});
  const [confirmingPickupByItem, setConfirmingPickupByItem] = useState<Record<number, boolean>>({});
  const [haggleModalVisible, setHaggleModalVisible] = useState(false);
  const [selectedHaggleItem, setSelectedHaggleItem] = useState<any>(null);
  ...
  const getMergedPricingFactors = (item: any) => {
    return {
  ...
  const handleConfirmPickedUp = (item: any) => {
    const orderItemId = resolveOrderItemId(item);
    if (!orderItemId) return;
        ...
        { text: 'Not Yet', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const result = await orderTrackingService.confirmOrderItemPickup(String(orderItemId));

              if (!result?.success) {
                Alert.alert('Error', result?.message || 'Failed to confirm pickup.');
                return;
              }
  ...
  const closeHaggleModal = () => {
    if (selectedHaggleItem?.order_item_id && submittingHaggleByItem[selectedHaggleItem.order_item_id]) {
      return;
    }
    setHaggleModalVisible(false);
    setSelectedHaggleItem(null);
    setHagglePriceInput('');
  };

  const handleHagglePrice = async (item: any) => {
    const orderItemId = resolveOrderItemId(item);
    if (!orderItemId) {
      Alert.alert('Error', 'Order item not found for haggling.');
      return;
    }
    ...
    setSelectedHaggleItem(item);
    setHagglePriceInput(Number.isFinite(defaultPrice) && defaultPrice > 0 ? defaultPrice.toFixed(2) : '');
    setHaggleModalVisible(true);
  };

  const submitHagglePrice = async () => {
    const item = selectedHaggleItem;
    const orderItemId = resolveOrderItemId(item);
    if (!orderItemId) {
      Alert.alert('Error', 'Order item not found for haggling.');
      return;
    ...
    if (!Number.isFinite(offeredPrice) || offeredPrice <= 0) {
      Alert.alert('Error', 'Please enter a valid haggle price.');
      return;
    }

    try {
      setSubmittingHaggleByItem((prev) => ({ ...prev, [orderItemId]: true }));
      const result = await orderTrackingService.hagglePrice(String(orderItemId), offeredPrice);

      if (!result?.success) {
        Alert.alert('Error', result?.message || 'Failed to submit haggle price.');
        return;
      }
      ...
      Alert.alert('Error', error?.message || 'Error submitting haggle price. Please try again.');
  ...
  const pickEnhancementPhotos = async () => {
    ...
    const next = result.assets.map((a, i) => ({
      uri: a.uri,
      name: (a as { fileName?: string }).fileName || `photo-${Date.now()}-${i}.jpg`,
      type: (a as { mimeType?: string }).mimeType || "image/jpeg",
    }));

    setEnhancementPhotos((prev) => {
      const merged = [...prev, ...next];
      const dedupedByUri = merged.filter((photo, index, arr) => arr.findIndex((p) => p.uri === photo.uri) === index);
      return dedupedByUri.slice(0, 5);
    });
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
                ...
                <View style={styles.orderCards}>
                  ...
                  return (
                    <View key={`${item.order_id}-${itemOrderItemId || item.child_order_id || item.order_item_id}`} style={styles.orderCard}>
                      ...
                      {showHaggleStatus && (
                        <View style={styles.haggleStatusRow}>
                          {hagglePending && (
                            <View style={[styles.haggleBadge, styles.haggleBadgePending]}>
                              <Text style={[styles.haggleBadgeText, styles.haggleBadgeTextPending]}>
                                Haggle Sent{hasHaggleOffer ? ` (${formatCurrencyPHP(haggleOffer)})` : ''}
                              </Text>
                            </View>
                          )}
                          {haggleAccepted && (
                            <View style={[styles.haggleBadge, styles.haggleBadgeAccepted]}>
                              <Text style={[styles.haggleBadgeText, styles.haggleBadgeTextAccepted]}>
                                Haggle Accepted{hasHaggleOffer ? ` (${formatCurrencyPHP(haggleOffer)})` : ''}
                              </Text>
                            </View>
                          )}
                          {haggleDeclined && (
                            <View style={[styles.haggleBadge, styles.haggleBadgeDeclined]}>
                              <Text style={[styles.haggleBadgeText, styles.haggleBadgeTextDeclined]}>
                                Haggle Declined{hasHaggleOffer ? ` (${formatCurrencyPHP(haggleOffer)})` : ''}
                              </Text>
                            </View>
                          )}
                        </View>
                      ...
                      {shouldShowPriceConfirmation(item) && (
                        <View style={styles.priceConfirmationActions}>
                          <View style={styles.confirmationMessage}>
                            ...
                            {haggleDeclined && (
                              <Text style={styles.haggleDeclinedText}>
                      ...
                      <View style={styles.orderFooter}>
                        ...
                        <View style={styles.orderFooterActions}>
                          {(item.status === 'ready_to_pickup' || item.status === 'ready_for_pickup') && item.service_type !== 'rental' && (
                            <TouchableOpacity
                              style={[styles.confirmPickupButton, confirmingPickupByItem[itemOrderItemId] && styles.confirmPickupButtonDisabled]}
                              onPress={() => handleConfirmPickedUp(item)}
                              disabled={!!confirmingPickupByItem[itemOrderItemId]}
                            >
                              <Ionicons name="checkmark-circle-outline" size={14} color="#fff" />
                              <Text style={styles.confirmPickupButtonText}>
                                {confirmingPickupByItem[itemOrderItemId] ? 'Confirming...' : 'Confirm Picked Up'}
                              </Text>
                            </TouchableOpacity>
                          )}
                          <TouchableOpacity
      ...
      <Modal
        visible={haggleModalVisible}
        transparent={true}
        animationType="fade"
        onRequestClose={closeHaggleModal}
      >
        <View style={styles.haggleModalOverlay}>
          <View style={styles.haggleModalCard}>
            <View style={styles.haggleModalHeader}>
              <Text style={styles.haggleModalTitle}>Haggle Price</Text>
              <TouchableOpacity onPress={closeHaggleModal}>
                <Ionicons name="close" size={22} color="#6B7280" />
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.haggleModalBody}>
              ...
              <TextInput
                ...
                onChangeText={setHagglePriceInput}
                keyboardType="numeric"
                placeholder="0.00"
                placeholderTextColor="#9CA3AF"
              />

              <View style={styles.haggleModalButtons}>
                <TouchableOpacity
                  style={styles.haggleCancelButton}
                  onPress={closeHaggleModal}
                  disabled={!!(selectedHaggleItem?.order_item_id && submittingHaggleByItem[selectedHaggleItem.order_item_id])}
                >
                  <Text style={styles.haggleCancelButtonText}>Cancel</Text>
                </TouchableOpacity>

                <TouchableOpacity
                  style={[
                    styles.haggleSubmitButton,
                    (selectedHaggleItem?.order_item_id && submittingHaggleByItem[selectedHaggleItem.order_item_id]) && styles.haggleSubmitButtonDisabled,
                  ]}
                  onPress={submitHagglePrice}
                  disabled={!!(selectedHaggleItem?.order_item_id && submittingHaggleByItem[selectedHaggleItem.order_item_id])}
                >
                  {selectedHaggleItem?.order_item_id && submittingHaggleByItem[selectedHaggleItem.order_item_id] ? (
                    <ActivityIndicator size="small" color="#fff" />
                  ) : (
                    <Text style={styles.haggleSubmitButtonText}>Submit Haggle</Text>
                  )}
                </TouchableOpacity>
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  btnHagglePriceDisabled: {
    backgroundColor: "#C7B29A",
  },
  btnHagglePriceText: {
    color: "#fff",
    fontWeight: "600",
  ...
  haggleBadgeTextPending: {
    color: "#7A4B00",
  },
  haggleBadgeTextAccepted: {
    color: "#1C6E3A",
  },
  haggleBadgeTextDeclined: {
    color: "#B91C1C",
  },
  ...
  haggleSubmitButtonDisabled: {
    opacity: 0.65,
  },
  haggleSubmitButtonText: {
    color: "#fff",
    fontSize: 14,
    fontWeight: "700",
  },
