...
export default function LoginScreen() {
      ...
      try {
        ...
        if (storedUser) {
          ...
          if (parsedUser?.username) {
            setEmail(String(parsedUser.username));
          }
        }
      } catch (error) {
        console.error('Error loading saved login username:', error);
      }
    };
  ...
  const resolveLoginIdentifier = async (value: string) => {
    ...
    try {
      ...
      if (storedUser) {
        const parsedUser = JSON.parse(storedUser);
        if (parsedUser?.email && String(parsedUser.email).toLowerCase() === trimmedValue.toLowerCase() && parsedUser?.username) {
          return String(parsedUser.username);
        }
      }
    } catch (error) {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const loginIdentifier = await resolveLoginIdentifier(email);
      ...
      if (response.token) {
        ...
        await AsyncStorage.setItem('lastLoginUsername', response.user?.username || loginIdentifier);
